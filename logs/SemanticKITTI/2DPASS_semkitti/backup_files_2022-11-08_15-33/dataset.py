...
class point_image_dataset_semkitti(data.Dataset):
    def __init__(self, in_dataset, config, loader_config, num_vote=1, trans_std=[0.1, 0.1, 0.1], max_dropout_ratio=0.2):
        'Initialization'
        self.point_cloud_dataset = in_dataset # the pc_dataset specific to point clouds
        self.config = config
    ...
    def __getitem__(self, index):
        ...
        # Calls __get_item__ of pc_dataset - Returns dict of point cloud, labeling, image and calibration info
        #                                   ...and the full path to the sample's .bin file
        data, root = self.point_cloud_dataset[index]

        xyz = data['xyz'] # (num points, 3) array of x, y, z coords for each point
        labels = data['labels'] # (num points, 1) array of labels for each point
        instance_label = data['instance_label'].reshape(-1) # (num points, 1) array of instance labels for each point
        sig = data['signal'] # (num points, 1) array of signal intensity values
        origin_len = data['origin_len'] # Int indicating number of points
        ...
        # Projects points into image
        keep_idx = xyz[:, 0] > 0  # Only keep point in front of the vehicle
        points_hcoords = np.concatenate([xyz[keep_idx], np.ones([keep_idx.sum(), 1], dtype=np.float32)], axis=1) # Points in homogeneous coords
        # Projection onto image plane and pixel coords
        ...
        data_dict = {}
        data_dict['point_feat'] = feat # Point cloud info ([x, y, z, signal] for every point) (masked)
        data_dict['point_label'] = labels # Array of labels in desired numbering format (masked)
