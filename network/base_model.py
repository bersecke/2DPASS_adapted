...
class LightningBaseModel(pl.LightningModule):
    ...
    def training_step(self, data_dict, batch_idx):
        # data_dict is a batch from the train/val dataloader in question, containing keys from collate:
        # dict_keys(['points', 'ref_xyz', 'batch_idx', 'batch_size', 'labels', 'raw_labels', 
        # 'origin_len', 'indices', 'point2img_index', 'img', 'img_indices', 'img_label', 'path'])
        ...
        self.train_acc(data_dict['logits'].argmax(1)[data_dict['labels'] != self.ignore_label],
            ...
            # data_dict["labels"]: tensor (e.g. Tensor([16, 16, 14, ..., 9, 9]) as long as the amount of captured points (e.g. (245180, 1])
            # data_dict["logits"]: tensor of shape (number of captured points, number of classes), e.g. (245180, 20)
