...
class SemanticKITTI(data.Dataset):
    def __init__(self, config, data_path, imageset='train', num_vote=1):
        ...
        # Determines selected data split
        if imageset == 'train':
        ...
        for i_folder in split:
            ...
            # Creates projection matrix (intrinsic & extrinsic info)
            proj_matrix = np.matmul(calib["P2"], calib["Tr"])
            self.proj_matrix[i_folder] = proj_matrix

        # Applies class weights to compensate imbalances
    ...
    def __getitem__(self, index):
        ...
        if self.imageset == 'test':
            ...
            instance_label = annotated_data >> 16 # Accessing upper 16 bits to get instance id (see http://www.semantic-kitti.org/dataset.html#format)
            annotated_data = annotated_data & 0xFFFF  # delete upper 16 digits binary to access lower 16 bits with label info
            # Converts to the desired label numeration in the semantic-kitti.yaml file
            # Format: array of shape (number of points, 1)
        ...
        # ... and respective projective matrices
        proj_matrix = self.proj_matrix[int(self.im_idx[index][-22:-20])]

        # Information packed in data_dict
        data_dict = {}
